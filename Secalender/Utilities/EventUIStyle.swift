...
struct DateTimePickerView: View {
    ...
    private var summaryText: String {
        ...
        df.locale = locale
        df.dateFormat = isAllDay ? "M月d日 EEE 整日" : "M月d日 EEE a h:mm"
        
        //修改内容：只看 isHasEnd 决定是否显示结束
        if !isAllDay, isHasEnd, let et = endTime, let ed = endDate {
            let mergedEnd = merge(date: ed, time: et)
            let tf = DateFormatter()
            tf.locale = locale
            tf.dateFormat = "a h:mm"
            return "\(df.string(from: mergedStart)) - \(tf.string(from: mergedEnd))"
        }
